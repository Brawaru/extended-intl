...
export default ({
  numbers: {
    nu: #(@numericSystems),
    decimal: {
      =[numericSystem: @numericSystems]: {
        =[style: decimalStylesFor @numericSystem]: {
          =[number: reducedNumbersFor 'decimal', @numericSystem, @style]: {
            =[pluralCase: pluralCasesFor 'decimal', @numericSystem, @style, @number]: \
              #(decimalInPluralCase @numericSystem, @style, @number, @pluralCase),
          },
        },
      },
    },
    currency: {
      =[numericSystem: @numericSystems]: {
        short: {
          =[number: reducedNumbersFor 'currency', @numericSystem, 'short']: {
            =[pluralCase: pluralCasesFor 'currency', @numericSystem, @style, @number]: \
              #(currencyInPluralCase @number, @pluralCase),
          },
        },
      }
    },
...
And then also auto-imports for every file that has:
```scaffold-js
import { addNumberFormatData } from '@braw/extended-intl';
import data from '@braw/extended-intl/locale-data/#(escapeString @locale)/data.js'
addNumberFormatData(#(asString @locale), data)
```
*/
...
//   K extends OnlyStrings<keyof T>,
// >(node: T, keys: K[]): Pick<T, K> {
//   const o = Object.create(Object.getPrototypeOf(node))

//   for (const key of Object.keys(node)) {
//     if (keys.includes(key as K)) o[key] = node[key]
//   }
...
function generateExporter(locale: string) {
  return `// prettier-ignore
import data from ${JSON.stringify(`./${locale}.data.mjs`)};
import { addNumberFormatData } from '../index.mjs';
addNumberFormatData(${JSON.stringify(locale)}, data);`
}

const dataFileDTS = `declare const _default: Parameters<
  typeof import('../index')['addNumberFormatData']
>[1]
...
export function generateLocaleData({
  ...
  for (const locale of locales) {
    ...
    output(`${locale}.mjs`, generateExporter(locale))
